...
config = {
    ...
    'vae_model': ".././TRAINING/models/vae.pth",
    'vae_scaler': ".././TRAINING/models/vae-scaler.pkl",
    'vae_layers_dim':[512, 256, 128, 64],
    'mapping_model': ".././TRAINING/models/mapping.pth",
    'output_cluster': [f"results_{mode_sam}/cluster_backbone", f"results_{mode_sam}/cluster_full", f"results_{mode_sam}/cluster_opt"],
...
if __name__ == "__main__":
    ...
    All_grad_distance = []
    Best_cvs_path = []
    json_dir = f"results_{mode_sam}/json"
    ...
    for di in range(len(dlis)):
        ...
        for t in step_lis:
            ...
            for i in range(len(cluster_idx_lis)):
                ...
                subprocess.run(f"Scwrl4 -i {cluster_backbone_path} -o {cluster_sidechain_path} -h -t > ./results_{mode_sam}/sidechain-{config['mode']}.log", shell=True, check=True)
            ...
            log_minim_ = f"results_{mode_sam}/minim"
            os.makedirs(log_minim_, exist_ok=True)

            full_pdb_files = [f for f in os.listdir(full_pdb_path) if f.endswith('.pdb')]
            subprocess.run(f"./minim.sh -i {full_pdb_path} -o {full_opt_path} -x {full_txt_path} -s {str(t)} -l {log_minim_} -n {Number_directions} > ./results_{mode_sam}/minim.log 2>&1", shell=True, check=True)
